...
describe('Database PRAGMAs', () => {
  it('applies busy_timeout in DatabaseManager initialization', async () => {
    ...
    const { DatabaseManager } = await import('../../../src/services/sqlite/Database.js');
  ...
  it('applies busy_timeout in direct connections', async () => {
    const { ClaudeMemDatabase } = await import('../../../src/services/sqlite/Database.js');
    const db = new ClaudeMemDatabase(':memory:');

    try {
      expect(getBusyTimeout(db.db)).toBe(SQLITE_BUSY_TIMEOUT_MS);
    } finally {
      db.close();
    }
  });
